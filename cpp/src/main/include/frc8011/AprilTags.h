...
class AprilTags {
 private:
  ...
  double botPoseX, botPoseZ, botPoseYawn, errAng, errX, errZ, speedAng, speedX,
      speedZ, lastErrA;
 ...
 public:
   ...
   */
  void April_Init() { table->PutNumber("pipeline", 0); }

  /**
   * @brief 获取视觉识别标志位
   */
  bool GetFlag() { return flag_vision; }

  /**
   * @brief 设置底盘瞄准前IMU数据，作为目标值
   */
  void SetAngle_0(double angle) { Angle_0 = angle; }

  /**
   * @brief 获取底盘瞄准前IMU数据
   */
  double GetAngle_0() { return Angle_0; }
  ...
  double GetError_inLeftRight() { return botPose.at(0); }
  /**
   * @brief 是否瞄准
   */
  bool isAiming() {
  ...
  double Get_AprilID_Vision() { return TagID.at(0); }
  double GetX_Vision() {
    return botPose.at(0);  // 面向tag的向右方为正（需取反
  }
  double GetY_Vision() { return botPose.at(1); }
  double GetZ_Vision() {
    return botPose.at(2);  // 面向tag向前为正
  }
  double GetPitch_Vision() { return botPose.at(3); }
  double GetYaw_Vision() {
    return botPose.at(4);  // yaw角，逆时针为负（需取反
  }
  double GetRoll_Vision() { return botPose.at(5); }

  double Getapriltag_id() { return tagid; }
