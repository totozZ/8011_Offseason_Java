 ...
 * @brief A header file for the Wayimotor class, which is a class for the motor control of the robot, which includes normal motor parameters and control methods.
 * @version 0.1
 * @date 2025-02-18
 * 
...
struct WayimotorConfig
{
    ...
    uint8_t mode = 0; // 电机控制模式： 0-制动模式，1-速度环模式，2-位置环模式，3-电流环模式，4-motionmagic模式
...
private:
    ...
    controls::MotionMagicVelocityTorqueCurrentFOC motionmagicvelocity =
      controls::MotionMagicVelocityTorqueCurrentFOC{0_tps}.WithUpdateFreqHz(500_Hz).WithSlot(0); // motionmagic控制速度
    ...
    controls::MotionMagicExpoVoltage mm_position = 
      controls::MotionMagicExpoVoltage{0_tr}.WithUpdateFreqHz(50_Hz).WithSlot(0).WithEnableFOC(1); // motionmagic控制位置

    controls::TorqueCurrentFOC Torque = controls::TorqueCurrentFOC{0_A}.WithUpdateFreqHz(500_Hz); // 电流控制

    controls::MotionMagicExpoTorqueCurrentFOC motionmagic = controls::MotionMagicExpoTorqueCurrentFOC{0_tr}.WithUpdateFreqHz(500_Hz).WithSlot(0); // motionmagic控制位置
...
public:
    ...
    void setMaxCurrent(double maxCur) {
        wayiconfig.maxCurrent = maxCur;
    }
    ...
    WayimotorConfig& Getdata() {
        return wayiconfig;
    } // 获取电机数据

    hardware::TalonFX& Getmotor() {
        return motor;
    } // 获取电机对象

    void setCurrent_Speed(double speed) {
        wayiconfig.Current_speed = speed;
    }
