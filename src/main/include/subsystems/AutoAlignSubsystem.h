...
namespace subsystems
{
    ...
    private:
        ...
        frc::Pose2d nearest_tag_pos;            // 距离机器人最近的Tag位置
        std::array<frc::Pose2d, 6> all_tag_pos; // 所有当前联盟的Tag位置
        double distance;                        // 目标位置到Tag的距离（根据预期方位决定）
        ...
        bool has_command = false; // Command相关，为False时没有正在执行的Command，为True时有正在执行的Command
