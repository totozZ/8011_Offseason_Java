 ...
 *
 * <p>
 * 真机安全：第一次测试必须架空车轮和机构、清空机器人周围，并安排一人随时按下 Driver Station 的
 * Disable（Enter）或 E-stop（空格）。系统会在 15 秒时强制结束整套自动并停止所有输出。
 *
 * <p>
 * 底盘使用机器人坐标系：Vx 正数向前，Vy 正数向左，rotation 正数逆时针。Vx/Vy 单位是 m/s，
 * rotation 单位是 °/s，时间单位是秒。系统会把平移合速度限制到 1.0 m/s，把旋转限制到 90°/s。
 * 两台车有无 Pigeon 都使用相同接口；动作按时间运行，不依赖陀螺仪。setRotation(45) 是每秒转 45° 的
 * 目标转速，不是转到 45°；边走边转时移动方向跟随车身变化，实际距离和角度需要实测。
 *
 * <p>
 * {@code sequence(a, b)} 表示先做 a 再做 b；{@code parallel(a, b)} 表示同时做 a 和 b；
 * {@code waitSeconds(1.0)} 表示等待 1 秒。不要在 parallel 中同时放两个底盘动作或两个机构动作。
 *
 * <p>
 * 常用动作：{@code robot.intakeFor(2.0)} 自动吸球 2 秒；{@code robot.shootFor(3.0)} 先预转 1
 * 秒，
 * 再射球 3 秒。它们结束时都会停止机构。
 *
 * <p>
 * 自由实验：{@code robot.intake().speed(0.5)}、{@code robot.shooter().speed(-0.7)}
 * 会设置并保持
