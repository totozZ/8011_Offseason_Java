...
void GroundIntakeSubsystem::Initialization()
{
  ...
  ctre::phoenix::StatusCode intake_roller_status = ctre::phoenix::StatusCode::StatusCodeNotInitialized;
  for (int i = 0; i < 5; ++i)
  {
  ...
  intake_pivot_slot0.kP = 0.;
  intake_pivot_slot0.kI = 0.;
  intake_pivot_slot0.kD = 0.;
  intake_pivot_slot0.GravityType = 0;

  ctre::phoenix::StatusCode intake_pivot_status = ctre::phoenix::StatusCode::StatusCodeNotInitialized;
  for (int i = 0; i < 5; ++i)
  {
...
void GroundIntakeSubsystem::Periodic()
{
  ...
  frc::SmartDashboard::PutNumber("ground_intake_roller_velocity", intake_roller_.Getdata().currentVelocity);
  frc::SmartDashboard::PutNumber("ground_intake_pivot_position", intake_pivot_.Getdata().currentPosition);
}
...
{
  intake_pivot_.setposition(position);
}
